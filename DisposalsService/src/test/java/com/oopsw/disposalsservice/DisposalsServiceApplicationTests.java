...
class DisposalsServiceApplicationTests {
    ...
    @Test void unauthenticatedRequestsAreRejected() throws Exception {
        assertThat(get("/disposals", null).statusCode()).isEqualTo(401);
    }
    @Test void authenticatedUnimplementedRouteIsExplicit() throws Exception {
        var result = get("/disposals", token(Instant.now().plusSeconds(60)));
        assertThat(result.statusCode()).isEqualTo(501);
        assertThat(result.body()).contains("ENDPOINT_NOT_IMPLEMENTED", "DisposalsService");
    }
    @Test void expiredTokensAreRejected() throws Exception {
        assertThat(get("/disposals", token(Instant.now().minusSeconds(60))).statusCode()).isEqualTo(401);
    }
