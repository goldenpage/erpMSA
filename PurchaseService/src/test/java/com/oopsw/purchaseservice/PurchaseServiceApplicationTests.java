...
class PurchaseServiceApplicationTests {
    ...
    @Test void unauthenticatedRequestsAreRejected() throws Exception {
        assertThat(get("/purchase", null).statusCode()).isEqualTo(401);
    }
    @Test void authenticatedUnimplementedRouteIsExplicit() throws Exception {
        var result = get("/purchase", token(Instant.now().plusSeconds(60)));
        assertThat(result.statusCode()).isEqualTo(501);
        assertThat(result.body()).contains("ENDPOINT_NOT_IMPLEMENTED", "PurchaseService");
    }
    @Test void expiredTokensAreRejected() throws Exception {
        assertThat(get("/purchase", token(Instant.now().minusSeconds(60))).statusCode()).isEqualTo(401);
    }
