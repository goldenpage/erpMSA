...
const users = new SharedArray('users', () => JSON.parse(open(__ENV.USERS_FILE || '/work/results/users.json')));
...
export default function () {
  ...
  const response = http.get(`${base}/foodmaterials?${query}`, {
    headers: { Authorization: `Bearer ${c.user.token}` },
    tags: { name: 'GET /foodmaterials', profile, route: base.includes('gateway') ? 'gateway' : 'direct' },
    timeout: '5s',
  });
  check(response, { '200 with correct tenant data': r => {
    if (r.status !== 200) return false;
    const body = r.json();
    return Array.isArray(body.foodMaterials) && body.foodMaterials.length > 0 && body.foodMaterials.every(item => item.sku.startsWith(`P-${c.user.accountId}-`));
  }});
}

export function handleSummary(data) {
  return { [`${__ENV.RESULTS_DIR || "/work/results"}/${label}.json`]: JSON.stringify(data, null, 2) };
}
