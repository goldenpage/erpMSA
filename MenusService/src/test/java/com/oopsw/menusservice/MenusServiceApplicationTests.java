...
class MenusServiceApplicationTests {
    ...
    @Test void unauthenticatedRequestsAreRejected() throws Exception {
        assertThat(get("/menus", null).statusCode()).isEqualTo(401);
    }
    @Test void authenticatedUnimplementedRouteIsExplicit() throws Exception {
        var result = get("/menus", token(Instant.now().plusSeconds(60)));
        assertThat(result.statusCode()).isEqualTo(501);
        assertThat(result.body()).contains("ENDPOINT_NOT_IMPLEMENTED", "MenusService");
    }
    @Test void expiredTokensAreRejected() throws Exception {
        assertThat(get("/menus", token(Instant.now().minusSeconds(60))).statusCode()).isEqualTo(401);
    }
