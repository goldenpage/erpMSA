...
| MenusService | 7072 | `/menus` | 음식 메뉴 CRUD·비활성화 |
| FoodMaterialsService | 7073 | `/foodmaterials` | 기초 카탈로그 CRUD·현재고·수량 조정·원장 |
...
| DisposalsService | 7077 | `/disposals` | 폐기 기록·재고 차감·재시도 |
...
FoodMaterials는 SKU·이름·설명·단가·상태·계정 소유권의 카탈로그와 식자재 재고를 관리한다. 단위·유통기한·레시피 등의 업무 기능을
...
재고의 현재고·수량 조정·원장은 FoodMaterials 내부 모듈로 이관했으며 `/foodmaterials/inventories`에서 제공한다.
기존 감사 저장·소비자 중복 방지·DLT 처리는 아직 복구하지 않았다.
...
새 환경은 FoodMaterials의 재고 저장을 위해 inventorydb를 생성한다. auditdb는 생성하지 않는다.
재고 migration V1은 이전 SQL을 바이트 단위로 보존하여 별도 Flyway 이력으로 관리한다.
2026-09-15의 [전환 검증](service-transition-results.md)은 제거 전 구성의 기록이다.
재고 이관의 현재 검증 범위는 [FoodMaterials 재고 이관](foodmaterials-inventory-migration.md)을 따른다.

## 신규 서비스의 인증과 미구현 계약

Menus·Disposals는 공개키 JWT 검증을 유지하며 실제 업무 API와 각각 menusdb·disposalsdb를 사용한다.
Menus는 기본 음식 메뉴 관리, Disposals는 폐기 기록과 FoodMaterials 재고 차감을 담당한다.
폐기 요청의 실패·재시도 계약은 [메뉴·폐기 구현](menus-disposals-implementation.md)을 따른다.
Notices·Bills·Purchase는 정상 Access Token에도 `501 ENDPOINT_NOT_IMPLEMENTED`를 반환한다.
Bills·Purchase는 사용자 요청에 따라 대기하며 Notices의 세부 업무는 아직 확정하지 않았다.
모든 업무 서비스의 개인키 접근은 금지하며 JWT 발행용 개인키는 Account만 받는다.

메뉴의 레시피 연결, 식자재 단위·유통기한, 공지 권한, Bills의 의미, Purchase의 입고 확정,
폐기 취소·반품·승인 흐름은 추가 요구사항 없이 구현하지 않는다.
...
1. `.env`의 `ITEM_DB_*`, `ITEM_SERVER_PORT`, `ITEM_FLYWAY_BASELINE_ON_MIGRATE`를 대응하는
   ...
   기존 `INVENTORY_DB_URL/USERNAME/PASSWORD`는 동일한 재고 DB를 가리키도록 `FOOD_MATERIAL_INVENTORY_DB_URL/USERNAME/PASSWORD`로 옮긴다.
   `AUDIT_*`, `KAFKA_AUDIT_GROUP_ID`, `KAFKA_ACCOUNT_LIFECYCLE_DLT_TOPIC`은 사용하지 않는다.
   Disposals는 `FOOD_MATERIAL_SERVICE_BASE_URL=http://FOODMATERIALSSERVICE`를 사용한다.
...
Jenkins는 Menus CRUD·Disposals 재고 차감/재시도, 미구현 3개 서비스의 501을 검사한다. 원격 CI 실행 여부는 별도 검증 결과에 표시한다.
